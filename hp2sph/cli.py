...
Run as an installed console script or as a module::

    hp2sph path/to/sky_map.fits
    python -m hp2sph path/to/sky_map.fits

The OpenMP guards are set automatically on import (see ``hp2sph/_bootstrap.py``),
...
def forward(mp: np.array, save: bool = False, **nufft_kw) -> np.array:
    ...
    ``mp`` is a SINGLE HEALPix intensity map (1-D); a 2-D input raises
    ``ValueError``. Pass ``mp[0]`` for the intensity row of an (I, Q, U) stack,
    and use :func:`hp2sph.forward_spin` for polarization.

    ``nufft_kw`` is forwarded to ``apply_nuFFT``. With ``save=True`` the result is
    also written to ``alm_array_cg_<nside>.fits`` in the current directory.
    """
    mp = np.asarray(mp)
    if mp.ndim != 1:
        raise ValueError(
            ...
            "hp2sph.spin_transform.forward_spin for polarization."
        )
